...
const currentUser = {
  ...
  nombre: null,
  rol: null,
  puede_importar_tabla: false,
  puede_borrar_tabla: false,
  puede_modificar_tabla: false,
  puede_desactivar_supabase: false,
};
...
const loadUserSession = async () => {
  ...
  if (session?.user?.email) {
    ...
    if (permissions) {
      currentUser.email = permissions.email;
      currentUser.nombre = permissions.nombre;
      currentUser.rol = permissions.rol;
      currentUser.puede_importar_tabla = permissions.puede_importar_tabla;
      currentUser.puede_borrar_tabla = permissions.puede_borrar_tabla;
      currentUser.puede_modificar_tabla = permissions.puede_modificar_tabla;
      currentUser.puede_desactivar_supabase = permissions.puede_desactivar_supabase;
    }
  }
};

const isAdmin = () => currentUser.rol === "admin";
const canImportTabla = () => currentUser.puede_importar_tabla;
const canBorrarTabla = () => currentUser.puede_borrar_tabla;
const canModificarTabla = () => currentUser.puede_modificar_tabla;
const canDesactivarSupabase = () => currentUser.puede_desactivar_supabase;
...
const updateAuthUI = () => {
  ...
  if (currentUser.email) {
    ...
    if (userInfo) {
      ...
      userEmail.textContent = currentUser.nombre || currentUser.email;
    }
  } else {
...
const applyUserPermissions = () => {
  // Restringir botones de importar/borrar en tabla de datos
  const importButtons = document.querySelectorAll("[data-import='tabla']");
  const clearButtons = document.querySelectorAll("[data-clear='tabla']");
  const addButtons = document.querySelectorAll("#btnAddActividad");
  
  importButtons.forEach(btn => {
    btn.style.display = canImportTabla() ? "" : "none";
  });
  
  clearButtons.forEach(btn => {
    btn.style.display = canBorrarTabla() ? "" : "none";
  });
  
  addButtons.forEach(btn => {
    btn.style.display = canModificarTabla() ? "" : "none";
  });
  
  // Restringir botón de Supabase
  const supabaseToggle = document.getElementById("btnSupabaseToggle");
  if (supabaseToggle) {
    supabaseToggle.disabled = !canDesactivarSupabase();
    if (!canDesactivarSupabase()) {
      supabaseToggle.title = "Solo el administrador puede desactivar Supabase";
      supabaseToggle.style.opacity = "0.5";
      supabaseToggle.style.cursor = "not-allowed";
    }
  }
};
...
const init = async () => {
  // Load user session and check auth
  await loadUserSession();
  updateAuthUI();
  initAuthUI();
  applyUserPermissions();
